...
export function TabGames({ language }: TabGamesProps) {
  ...
  const modeLabel = (id: GameMode) => id === 'colors' ? ui.gameColors[language] : ui.gameShapes[language];
  ...
  return (
    <div className="px-4 pt-4 pb-36 max-w-md mx-auto">
      ...
      {visibleModes.length === 0 ? (
        ...
        <>
          {/* Mode switch — only show visible modes */}
          <div className="flex gap-1 mb-5 bg-muted rounded-full p-1">
            {visibleModes.map(({ id }) => (
              <button
                key={id}
                onClick={() => switchMode(id)}
                className={`flex-1 py-2 rounded-full text-sm font-bold transition-all duration-200 flex items-center justify-center gap-2 ${
                  mode === id
